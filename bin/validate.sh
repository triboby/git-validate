...
run_hook() {
    ...
    local projects; projects=$(find . -not -iwholename '*node_modules*' -not -iwholename '*lib*' -maxdepth 3 -name package.json -print 2>/dev/null)
    if [[ $? -ne 0 ]]; then
        projects=$(find . -not -ipath '*node_modules*' -not -ipath '*lib*' -maxdepth 3 -name package.json -print 2>/dev/null)
    fi
